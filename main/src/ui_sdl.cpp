...
std::string faceForMood(const std::string& mood) {
    ...
    if (mood == "bored2") return "(—__—)";
    if (mood == "intense" || mood == "busy") return "(°▃▃°)";
    if (mood == "intense2") return "(°ロ°)";
    if (mood == "cool" || mood == "listening-scanning") return "(⌐■_■)";
    if (mood == "cool2") return "(단__단)";
    if (mood == "happy" || mood == "happy-handshake" || mood == "grateful") return "(•‿‿•)";
    if (mood == "happy2") return "(^‿‿^)";
    if (mood == "happy3") return "(^◡◡^)";
    if (mood == "excited" || mood == "motivated") return "(ᵔ◡◡ᵔ)";
    if (mood == "motivated2") return "(★‿★)";
    if (mood == "motivated3") return "(•̀ᴗ•́)";
    if (mood == "demotivated") return "(≖__≖)";
    if (mood == "demotivated2") return "(￣ヘ￣)";
    if (mood == "demotivated3") return "(¬､¬)";
    if (mood == "smart") return "(✜‿‿✜)";
    if (mood == "friend") return "(♥‿‿♥)";
    if (mood == "friend2") return "(♡‿‿♡)";
    if (mood == "friend3") return "(♥‿♥ )";
    if (mood == "friend4") return "(♥ω♥ )";
    if (mood == "lonely") return "(ب__ب)";
    if (mood == "lonely2") return "(｡•́︿•̀｡)";
    if (mood == "lonely3") return "(︶︹︺)";
    if (mood == "sad") return "(╥☁╥ )";
    if (mood == "sad2") return "(╥﹏╥)";
    if (mood == "sad3") return "(ಥ﹏ಥ)";
    if (mood == "angry") return "(-_-')";
    if (mood == "angry2") return "(⇀__⇀)";
    if (mood == "angry3") return "(`___´)";
